...
function Backup([string]$path){ $bak=$path+'.bak_media3_'+(Get-Date -Format yyyyMMdd_HHmmss); Copy-Item -Force -LiteralPath $path -Destination $bak; return $bak }
...
Log '=== START FIX (video_player_android Media3) ==='
...
$vpGradle = Join-Path (Join-Path $pub 'video_player_android-2.9.1') 'android\build.gradle'
if(!(Test-Path -LiteralPath $vpGradle)){ Fail ('Target not found: '+$vpGradle) }
...
$L += ''
$L += 'dependencies {'
$L += ('  def media3 = '+$dq+$media3+$dq)
$L += ('  implementation '+$dq+'androidx.media3:media3-common:'+$media3+$dq)
$L += ('  implementation '+$dq+'androidx.media3:media3-exoplayer:'+$media3+$dq)
$L += ('  implementation '+$dq+'androidx.media3:media3-datasource:'+$media3+$dq)
$L += ('  implementation '+$dq+'androidx.media3:media3-exoplayer-hls:'+$media3+$dq)
$L += ('  implementation '+$dq+'androidx.media3:media3-exoplayer-dash:'+$media3+$dq)
$L += ('  implementation '+$dq+'androidx.media3:media3-exoplayer-rtsp:'+$media3+$dq)
$L += ('  implementation '+$dq+'androidx.media3:media3-ui:'+$media3+$dq)
$L += '}'
...
if(-not ($check -match 'androidx\.media3:media3-exoplayer')){ Fail ('Missing media3-exoplayer dep after rewrite: '+$vpGradle) }
Log '[FIXED] video_player_android build.gradle (Media3 deps present)'
Log ('        backup: '+$bak)
