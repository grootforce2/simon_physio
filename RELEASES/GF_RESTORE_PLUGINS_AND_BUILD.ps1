...
foreach($t in $targets){
  ...
  $bak = LatestBak $g
  if(-not $bak){
    Log ('[WARN] No backup found for: '+$g)
    continue
  }
...
if($restored -lt 2){
  Log '[WARN] Not all backups were found/restored.'
  Log '      If build still fails, we will hard-refresh those packages from pub cache in the next step.'
}
